<section class="py-12">
    <div class="max-w-7xl mx-auto px-3 xs:px-4 sm:px-8 lg:px-10">
        ...
        <div class="mt-10 sm:mt-12 grid min-[320px]:grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 sm:gap-6">
            ...
            <a href="{{ route('stores.show', $store->slug) }}" class="group relative flex flex-col items-center rounded-2xl bg-white p-5 border border-gray-200 shadow-sm transition hover:-translate-y-1 hover:shadow-lg hover:border-emerald-200">
                ...
                <span class="mt-1.5 rounded-full px-2.5 py-0.5 font-Inter text-xs font-semibold text-emerald-600">{{ $store->offers_count }} {{ $store->offers_count == 1 ? 'coupon' : 'coupons' }}</span>
            </a>
            @empty
            <div class="col-span-full text-center py-8">
                <p class="font-Inter text-gray-400">No new stores yet.</p>
            </div>
            @endforelse
        </div>
    </div>
