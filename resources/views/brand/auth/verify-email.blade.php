...
<body class="font-Inter">
    <x-auth-card title="Verify Your Email" subtitle="We've sent a 6-digit code to your email address">
        @session('status')
        ...
        <form method="POST" action="{{ route('brand.verify-email.verify') }}" class="space-y-4">
            @csrf
            ...
            <x-primary-button>Verify Email</x-primary-button>
        </form>

        <form method="POST" action="{{ route('brand.verify-email.resend') }}" class="mt-4">
            @csrf
