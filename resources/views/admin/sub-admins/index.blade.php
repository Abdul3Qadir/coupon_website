<x-layouts.admin title="Sub-Admins">
    <div class="flex items-center justify-between flex-wrap gap-3">
        <div>
            <h1 class="font-Manrope text-xl sm:text-2xl font-extrabold text-gray-900">Sub-Admins</h1>
            <p class="mt-1 font-Inter text-sm text-gray-500">Review access requests and manage permissions</p>
        </div>
        ...
        <form method="GET" class="flex items-center gap-2 rounded-full border border-gray-200 bg-white px-4 py-2">
            ...
            <input type="text" name="q" value="{{ $search }}" placeholder="Search sub-admins..." class="bg-transparent outline-none font-Inter text-sm text-gray-700 placeholder:text-gray-400">
        </form>
    </div>
    ...
    <div class="mt-6 flex items-center gap-2 overflow-x-auto">
        @foreach (['all' => 'All', 'pending' => 'Pending', 'approved' => 'Approved', 'rejected' => 'Rejected'] as $key => $label)
            <a href="{{ route('admin.sub-admins.index', ['status' => $key]) }}" @class([
        ...
        <div class="overflow-x-auto custom-scrollbar">
            <table class="w-full text-left">
                <thead class="border-b border-gray-100 bg-gray-50/50">
                    <tr>
                        <th class="px-5 py-3 font-Inter text-xs font-semibold uppercase tracking-wide text-gray-500">Name</th>
                        <th class="px-5 py-3 font-Inter text-xs font-semibold uppercase tracking-wide text-gray-500">Offers Added</th>
                        <th class="px-5 py-3 font-Inter text-xs font-semibold uppercase tracking-wide text-gray-500">Blogs Written</th>
                        <th class="px-5 py-3 font-Inter text-xs font-semibold uppercase tracking-wide text-gray-500">Status</th>
                        <th class="px-5 py-3 font-Inter text-xs font-semibold uppercase tracking-wide text-gray-500">Requested</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse ($subAdmins as $subAdmin)
                        <tr class="hover:bg-gray-50/50 transition">
                            ...
                            <td class="px-5 py-3.5 font-Inter text-sm text-gray-600">{{ $subAdmin->offers_count }}</td>
                            <td class="px-5 py-3.5 font-Inter text-sm text-gray-600">{{ $subAdmin->blogs_count }}</td>
                            <td class="px-5 py-3.5"><x-status-badge :status="$subAdmin->status" /></td>
                            <td class="px-5 py-3.5 font-Inter text-sm text-gray-500">{{ $subAdmin->created_at->format('M j, Y') }}</td>
                            <td class="px-5 py-3.5 text-right">
                                <a href="{{ route('admin.sub-admins.show', $subAdmin) }}" class="font-Inter text-sm font-semibold text-red-600 hover:text-red-700">View</a>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="6" class="px-5 py-10 text-center font-Inter text-sm text-gray-400">No sub-admins found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
