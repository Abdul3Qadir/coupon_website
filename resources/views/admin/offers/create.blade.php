<x-layouts.admin title="Add Offer">
    <div class="max-w-3xl mx-auto">
        <div class="mb-8">
            ...
            <h1 class="mt-4 font-Manrope text-2xl sm:text-3xl font-extrabold text-gray-900">Add New Offer</h1>
            <p class="mt-1 font-Inter text-sm text-gray-500">Create a coupon or deal for a brand</p>
        </div>

        <div class="rounded-2xl bg-white border border-gray-200 p-5 sm:p-8">
            @if (session('error'))
                <div class="mb-6 rounded-xl bg-red-50 border border-red-200 px-4 py-3 font-Inter text-sm font-medium text-red-700">
                    {{ session('error') }}
                </div>
            @endif
            ...
            <form method="POST" action="{{ route('admin.offers.store') }}">
                ...
                <div class="mb-6">
                    <label class="block font-Inter text-sm font-semibold text-gray-900 mb-2">Select Brand <span class="text-red-500">*</span></label>
                    <select name="brand_id" required class="custom-select block w-full rounded-xl border border-gray-200 bg-white px-4 py-3 font-Inter text-sm text-gray-900 outline-none focus:border-red-300 focus:ring-2 focus:ring-red-100 transition cursor-pointer">
                        <option value="">Choose a brand...</option>
                        @foreach ($brands as $brand)
                            <option value="{{ $brand->id }}" @selected(old('brand_id') == $brand->id)>{{ $brand->name }}</option>
                        @endforeach
                    </select>
                    @error('brand_id')<p class="mt-1.5 font-Inter text-xs font-medium text-red-600">{{ $message }}</p>@enderror
                </div>
                ...
                <div>
                    ...
                    <div class="grid grid-cols-2 gap-3 sm:gap-4">
                        <label class="cursor-pointer">
                            <input type="radio" name="type" value="coupon" class="peer sr-only" @checked(old('type', 'coupon') === 'coupon')>
                        ...
                        <label class="cursor-pointer">
                            <input type="radio" name="type" value="deal" class="peer sr-only" @checked(old('type') === 'deal')>
                            <div class="rounded-xl border-2 border-gray-200 peer-checked:border-red-500 peer-checked:bg-red-50 px-3 py-4 sm:px-4 sm:py-5 text-center transition hover:border-gray-300 h-full flex flex-col items-center justify-center">
                ...
                <div class="mt-6">
                    ...
                    <input type="text" name="title" value="{{ old('title') }}" placeholder="e.g. 20% Off All Summer Styles" required class="mt-2 block w-full rounded-xl border border-gray-200 bg-white px-4 py-3 font-Inter text-sm text-gray-900 outline-none focus:border-red-300 focus:ring-2 focus:ring-red-100 transition">
                ...
                <div class="mt-6 grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <select name="discount_type" required class="custom-select mt-2 block w-full rounded-xl border border-gray-200 bg-white px-4 py-3 font-Inter text-sm text-gray-900 outline-none focus:border-red-300 focus:ring-2 focus:ring-red-100 transition cursor-pointer">
                            <option value="percentage" @selected(old('discount_type') === 'percentage')>Percentage Off</option>
                            <option value="fixed" @selected(old('discount_type') === 'fixed')>Fixed Amount Off</option>
                            <option value="free_shipping" @selected(old('discount_type') === 'free_shipping')>Free Shipping</option>
                        </select>
                    </div>
                    <div>
                        <label class="block font-Inter text-sm font-semibold text-gray-900">Value</label>
                        <input type="number" name="discount_value" min="0" step="0.01" value="{{ old('discount_value') }}" placeholder="e.g. 20" class="mt-2 block w-full rounded-xl border border-gray-200 bg-white px-4 py-3 font-Inter text-sm text-gray-900 outline-none focus:border-red-300 focus:ring-2 focus:ring-red-100 transition">
                    </div>
                </div>

                <div id="codeFieldWrapper" class="mt-6">
                    <label class="block font-Inter text-sm font-semibold text-gray-900">Coupon Code</label>
                    <input id="codeInput" type="text" name="code" value="{{ old('code') }}" placeholder="e.g. SUMMER20" class="mt-2 block w-full rounded-xl border border-gray-200 bg-white px-4 py-3 font-Inter text-sm text-gray-900 uppercase outline-none focus:border-red-300 focus:ring-2 focus:ring-red-100 transition">
                    @error('code')<p class="mt-1.5 font-Inter text-xs font-medium text-red-600">{{ $message }}</p>@enderror
                </div>

                <div class="mt-6">
                    <label class="block font-Inter text-sm font-semibold text-gray-900">Redirect URL <span class="text-red-500">*</span></label>
                    <input type="url" name="redirect_url" value="{{ old('redirect_url') }}" placeholder="https://yourstore.com/sale" required class="mt-2 block w-full rounded-xl border border-gray-200 bg-white px-4 py-3 font-Inter text-sm text-gray-900 outline-none focus:border-red-300 focus:ring-2 focus:ring-red-100 transition">
                ...
                <div class="mt-6 grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="date" name="starts_at" value="{{ old('starts_at') }}" class="mt-2 block w-full rounded-xl border border-gray-200 bg-white px-4 py-3 font-Inter text-sm text-gray-900 outline-none focus:border-red-300 focus:ring-2 focus:ring-red-100 transition">
                    </div>
                    <div>
                        ...
                        <input type="date" name="expires_at" value="{{ old('expires_at') }}" class="mt-2 block w-full rounded-xl border border-gray-200 bg-white px-4 py-3 font-Inter text-sm text-gray-900 outline-none focus:border-red-300 focus:ring-2 focus:ring-red-100 transition">
                ...
                <div class="mt-8 pt-6 border-t border-gray-100 grid grid-cols-1 sm:grid-cols-2 gap-3">
                    ...
                    <a href="{{ route('admin.offers.index') }}" class="w-full inline-flex items-center justify-center rounded-xl bg-gray-100 px-6 py-3 font-Inter text-sm font-semibold text-gray-700 hover:bg-gray-200 transition">
                        Cancel
                    </a>
                </div>
            </form>
