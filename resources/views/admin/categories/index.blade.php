<x-layouts.admin title="Categories">
    <div class="flex items-center justify-between flex-wrap gap-3">
        <div>
            <h1 class="font-Manrope text-xl sm:text-2xl font-extrabold text-gray-900">Categories</h1>
            <p class="mt-1 font-Inter text-sm text-gray-500">Manage coupon and store categories</p>
        </div>
        <a href="{{ route('admin.categories.create') }}" class="inline-flex items-center gap-1.5 rounded-full bg-red-600 hover:bg-red-700 px-5 py-2.5 font-Manrope text-sm font-bold text-white transition">
    ...
    <div class="mt-6 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
        @forelse ($categories as $category)
            <div class="rounded-2xl border border-gray-200 bg-white p-5">
                <div class="flex items-start justify-between">
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <form method="POST" action="{{ route('admin.categories.destroy', $category) }}">
                            @csrf
                            @method('DELETE')
                            <button type="button" class="confirm-action cursor-pointer text-gray-400 hover:text-red-600 transition" data-confirm-title="Delete this category?" data-confirm-message="This can't be undone. Categories in use by brands or offers can't be deleted." data-confirm-button="Yes, Delete">
                ...
                <p class="font-Inter text-xs text-gray-500">{{ $category->brands_count }} brands · {{ $category->offers_count }} offers</p>
            </div>
        @empty
            <p class="col-span-full py-10 text-center font-Inter text-sm text-gray-400">No categories yet.</p>
        @endforelse
    </div>
</x-layouts.admin>