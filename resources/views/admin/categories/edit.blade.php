<x-layouts.admin title="Edit Category">
    <div class="flex items-center gap-1.5 font-Inter text-sm text-gray-500 mb-5">
        ...
        <span>/</span>
        <span class="text-gray-900 font-semibold">Edit {{ $category->name }}</span>
    </div>

    <div class="max-w-2xl rounded-2xl border border-gray-200 bg-white p-6">
        <h1 class="font-Manrope text-lg font-extrabold text-gray-900">Edit Category</h1>

        <form method="POST" action="{{ route('admin.categories.update', $category) }}" class="mt-5">
            @csrf
            @method('PUT')
