<x-layouts.admin title="Notifications">
    <div class="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        @if($unreadCount > 0)
            <form method="POST" action="{{ route('admin.notifications.mark-all-read') }}">
                @csrf
                <button type="submit" class="inline-flex items-center gap-1.5 rounded-xl bg-gray-900 px-4 py-2.5 cursor-pointer font-Inter text-sm font-semibold text-white hover:bg-gray-800 transition">
    ...
    <div class="mb-6 flex items-center gap-2 overflow-x-auto scrollbar-hide pb-1">
        <a href="{{ route('admin.notifications.index', ['filter' => 'all']) }}" @class([
        ...
        <a href="{{ route('admin.notifications.index', ['filter' => 'unread']) }}" @class([
